document.addEventListener('DOMContentLoaded', () => {
    ...
    const grid = document.getElementById('aventura-grid');
    ...
    if (!grid || !genreFilter || !sortBy || !gridViewBtn || !listViewBtn) {
        console.warn("Algunos elementos UI no se encontraron en aventura.html. Posible disparidad de IDs.");
        if (!grid) return;
    }
    ...
    const loadContent = () => {
        ...
        if (window.peliculas) {
            ...
            contentList = window.peliculas.filter(p => {
                ...
                return cats.map(t => String(t).toLowerCase()).includes('aventura');
            });
        }
    };
    ...
    if (window.peliculas && window.peliculas.length > 0) {
        ...
        document.addEventListener('app-ready', () => {
            console.log("Evento 'app-ready' recibido en aventura.js. Inicializando...");
            init();
        });
    }
});
