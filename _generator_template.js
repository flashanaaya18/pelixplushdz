document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!grid || !genreFilter || !sortBy || !gridViewBtn || !listViewBtn) {
        console.warn("Algunos elementos UI no se encontraron en {PAGE_DESC}. Posible disparidad de IDs.");
        if(!grid) return;
    }
    ...
    if(window.peliculas && window.peliculas.length > 0) {
        ...
        document.addEventListener('app-ready', () => {
            console.log("Evento 'app-ready' recibido en {SCRIPT_NAME}. Inicializando...");
            init();
        });
    }
});
