document.addEventListener('DOMContentLoaded', function() {
    ...
    const grid = document.getElementById('romance-grid');
    ...
    const loadContent = () => {
        ...
        if (listaPeliculas.length > 0) {
            ...
            // Filtrar contenido de Romance
            contentList = listaPeliculas.filter(p => {
                if (p.esta_roto) return false;
                
                // Lógica robusta para detectar romance en género o categoría
                let tags = [];
                ...
                const keywords = ['romance', 'amor', 'romantica', 'love'];
    ...
    const populateGenreFilter = () => {
        ...
        genres.sort().forEach(genre => {
            // Excluir 'romance' del filtro porque ya estamos en esa página
    ...
    const renderContent = () => {
        ...
        if (filtrados.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 50px; color: #888;">
                <h3>No se encontraron títulos de romance.</h3>
            </div>`;
        } else {
    ...
    if (window.peliculas && window.peliculas.length > 0) {
        ...
        document.addEventListener('app-ready', () => {
            console.log("Romance: App lista, inicializando...");
            init();
        });
