document.addEventListener('DOMContentLoaded', function() {
    ...
    const grid = document.getElementById('suspenso-grid');
    ...
    const loadContent = () => {
        ...
        if (listaPeliculas.length > 0) {
            ...
            // Filtrar contenido de Suspenso
            contentList = listaPeliculas.filter(p => {
                if (p.esta_roto) return false;
                
                // Lógica robusta para detectar suspenso en género o categoría
                let tags = [];
                ...
                const keywords = ['suspenso', 'thriller'];
    ...
    const populateGenreFilter = () => {
        ...
        genres.sort().forEach(genre => {
            // Excluir 'suspenso' del filtro porque ya estamos en esa página
            if (genre && !['suspenso', 'thriller'].includes(genre)) {
    ...
    const renderContent = () => {
        ...
        if (filtrados.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 50px; color: #888;">
                <h3>No se encontraron títulos de suspenso.</h3>
            </div>`;
        } else {
    ...
    if (window.peliculas && window.peliculas.length > 0) {
        ...
        document.addEventListener('app-ready', () => {
            console.log("Suspenso: App lista, inicializando...");
            init();
        });
